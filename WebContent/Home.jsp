...
<script type="text/javascript">
	...
	function validate_data() {
		...
		var quantity = document.forms["Expense_Data"]["quantity_purchased"].value;
		var amt = document.forms["Expense_Data"]["total_price"].value;

		if (quantity != "") {
			if (!decimalOnly.test(quantity)) {
				alert("Please Enter Number/Decimal values only of 2 Digit in the Quantity Field");
				return false;
			}
		}
		...
		if (amt != "") {
			if (!decimalOnly.test(amt)) {
				alert("Please Enter Number/Decimal values only of 2 Digit in the Amount Field");
				return false;
			}
		}
